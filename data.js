...
export const getProductData = (array) => {
    return array.map((item, index) => {
        const [productId, mealCategoryID, mealTypeID, mealCount] = item.split("-")


        console.log("productId ===>")
        const [product] = products.filter(item => item.productId === +productId);

        const mealType = product.mealTypes[+mealTypeID]
        const mealCategory = product.mealCategories[+mealCategoryID]
        const name = product.name[mealType]
        const [priceDetail] = product.priceDetails[mealType].filter(item => item.mealCount === +mealCount)
        return {
            name,
            mealCategory,
            mealType,
