...
export const ProductCard = ({ data }) => {
  ...
  const { cartDetails, setCartDetails } = React.useContext(cartContext);
  ...
  const onClickHandle = () => {
    ...
    const generatedProuductId = `${productId}-${mealCategories.indexOf(foodCategory)}-${mealTypes.indexOf(mealType)}-${selectedMeal}`;

    setCartDetails([...cartDetails, generatedProuductId])
    history.push("/cart")
  };
